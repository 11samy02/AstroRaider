...
const PROJECTILE = preload("res://scenes/projectiles/Photon_gun_projectile.tscn")
...
func attack() -> void:
	if is_instance_valid(enemy_detector_area.Enemy_list[0]):
		...
		projectile.atk_resource = atk_res.duplicate() if atk_res != null else AttackResource.new()
		projectile.atk_resource.source_type = AttackResource.SourceType.BUILDING
