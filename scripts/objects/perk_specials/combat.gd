...
func get_best_target() -> Node2D:
	...
	for enemy: Node2D in _get_attack_targets():
		...
		var target_velocity := _get_target_velocity(enemy)
		var to_protect := protect_pos - enemy.global_position
		if to_protect.length() > 0.001 and target_velocity.length() > 0.1:
			score += target_velocity.normalized().dot(to_protect.normalized()) * approach_weight
...
func _try_shoot(target: Node2D) -> void:
	if _shot_timer > 0.0:
		return
