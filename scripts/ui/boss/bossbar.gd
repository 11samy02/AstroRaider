...
func setup(new_boss: Node2D, new_boss_stats: BossStats) -> void:
	Boss = new_boss
	boss_stats = new_boss_stats
	_apply_boss_stats()


func _apply_boss_stats() -> void:
	if not is_instance_valid(boss_stats):
		return
	if not is_instance_valid(bar) or not is_instance_valid(chip_bar):
		return
