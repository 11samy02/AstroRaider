...
func _ready() -> void:
	SettingsSaver.apply_video_settings()
	
	selected_monitor = SettingsSaver.get_saved_monitor()
	selected_monitor = clamp(selected_monitor, 0, DisplayServer.get_screen_count() - 1)
...
func change_display_mode(goes_forward: bool) -> void:
	...
	SettingsSaver.save_display_mode(display_modes[index])
...
func change_resolution(goes_forward: bool) -> void:
	...
	SettingsSaver.save_resolution(new_size)
	SettingsSaver.save_display_mode(DisplayServer.WINDOW_MODE_WINDOWED)
...
func change_monitor(goes_forward: bool) -> void:
	...
	DisplayServer.window_set_current_screen(selected_monitor)
	SettingsSaver.save_monitor(selected_monitor)
...
func change_fps_limit(goes_forward: bool) -> void:
	...
	SettingsSaver.save_fps_limit(fps_limits[index])
	update_fps_text()
...
func vsync_checkbox_pressed(toggle_on: bool) -> void:
	...
	SettingsSaver.save_vsync_enabled(toggle_on)
