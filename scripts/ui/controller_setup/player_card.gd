...
func assign_controller(controller: int) -> void:
	...
	icon.texture = load("res://assets/sprites/ui/controller_select/controller.png")

func clear_controller() -> void:
	connected = false
	assigned_controller = -1
	label.text = "Press Start"
	icon.texture = load("res://assets/sprites/ui/controller_select/controller_inaktiv.png")
