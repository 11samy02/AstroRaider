...
const HEAL_PARTICLE := preload("res://scenes/particles/heal_particle.tscn")

@export var game_ui: GameUi
@export var chip_bar: TextureProgressBar
@export var health_label: Label
@export var gray_scale: ColorRect
@export_range(0, 1, 0.01) var diff_ratio_percentage := 0.08
@export_range(0, 1, 0.01) var glow_threshold := 0.25
@export_range(0.0, 1.0, 0.01) var grayscale_start_threshold := 0.33
@export_range(0.0, 1.0, 0.01) var grayscale_start_strength := 0.33
@export_range(0.01, 2.0, 0.01) var grayscale_tween_duration := 0.25
...
var grayscale_tween: Tween = null
var current_grayscale_strength := 0.0
var target_grayscale_strength := -1.0
...
func _check_game_over() -> void:
	...
	if player_res.current_health <= 0:
		...
		if GlobalGame.Players.is_empty() and !game_over:
			game_over = true
			var suit_id = player_res.player.selected_suit
			SuitExpRunTracker.apply_to_suit(SuitData.load_suit_res(suit_id))
...
## Updates grayscale strength based on current health
func _update_grayscale_from_health() -> void:
	if max_value <= 0:
		_tween_grayscale_strength(0.0)
		return

	var health_percent = clamp(float(value) / float(max_value), 0.0, 1.0)
	var new_target := 0.0

	if health_percent <= grayscale_start_threshold:
		var t := inverse_lerp(grayscale_start_threshold, 0.0, health_percent)
		new_target = lerp(grayscale_start_strength, 1.0, t)

	_tween_grayscale_strength(new_target)


## Smoothly tweens grayscale strength
func _tween_grayscale_strength(new_target: float) -> void:
	new_target = clamp(new_target, 0.0, 1.0)

	if abs(target_grayscale_strength - new_target) < 0.001:
		return

	target_grayscale_strength = new_target

	if grayscale_tween and grayscale_tween.is_valid():
		grayscale_tween.kill()

	grayscale_tween = create_tween()
	grayscale_tween.set_trans(Tween.TRANS_SINE)
	grayscale_tween.set_ease(Tween.EASE_OUT)
	grayscale_tween.tween_method(_apply_grayscale_strength, current_grayscale_strength, target_grayscale_strength, grayscale_tween_duration)


## Applies grayscale strength to the screen shader
func _apply_grayscale_strength(new_value: float) -> void:
	current_grayscale_strength = clamp(new_value, 0.0, 1.0)
	set_grayscale_strength(current_grayscale_strength)


## Sets grayscale shader strength
func set_grayscale_strength(value: float) -> void:
	if !is_instance_valid(gray_scale):
		return

	var mat := gray_scale.material as ShaderMaterial

	if mat == null:
		return

	mat.set_shader_parameter("grayscale_strength", clamp(value, 0.0, 1.0))
