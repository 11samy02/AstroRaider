...
## Handles always-active perks and resets stats if not selected in the current run.
func _process(_delta: float) -> void:
	if not is_instance_valid(perk_res):
		return
...
## Returns true if player, stats and perk resource are available.
func has_valid_runtime_refs() -> bool:
	return is_instance_valid(player) and is_instance_valid(stats) and is_instance_valid(perk_res)


## Returns the modifier id used by this perk.
func get_perk_modifier_id() -> String:
	return "perk_%s" % str(Key)
...
## Returns the main value of the perk as percentage.
## Example: value 20 becomes 0.2.
func get_value_percent() -> float:
...
## Returns the currently selected suit data from the player.
...
## Returns the modifier id used by the selected suit's level bonuses.
func get_selected_suit_modifier_id() -> String:
	var suit_data := get_selected_suit_data()

	if not is_instance_valid(suit_data):
		return ""

	return suit_data.get_suit_modifier_id()


## Returns true if a selected suit modifier id exists.
func has_selected_suit_modifier_id() -> bool:
	return get_selected_suit_modifier_id() != ""
...
## Returns true if this perk is an always-active passive perk.
func is_always_active() -> bool:
	return is_instance_valid(perk_res) and perk_res.active_type == Perk.Active_type_keys.Always
...
func get_player_res() -> PlayerResource:
	if not is_instance_valid(_player_res):
