...
var _target_enemy: EnemyBaseTemplate = null
...
func _find_priority_enemy() -> EnemyBaseTemplate:
...
func _is_valid_env_collider(collider: Variant) -> bool:
	...
	if collider is Node:
		...
		for enemy: EnemyBaseTemplate in GlobalGame.Enemies:
			if not is_instance_valid(enemy):
				continue
			if node == enemy or enemy.is_ancestor_of(node) or node.is_ancestor_of(enemy):
				return false
...
func _apply_impact(target_enemy: EnemyBaseTemplate) -> void:
	if not is_instance_valid(_player):
		return
...
func _damage_enemy(enemy: EnemyBaseTemplate) -> void:
	if not is_instance_valid(enemy):
		return
